...
from bcDiscovery import meausreBCNFLevel
import time
import numpy as np
...
def findFDMetricsPerTable(tableName, testAttributes, displayResults):
    ...
    if displayResults==1:
        printPKResult(tableName, primaryKey)
    if primaryKey!=[]:
...
def printPKResult(tableName, primaryKey):
    ...
    print(np.array(primaryKey[0]))
    print("The metric degree for this primary key is: "+str(primaryKey[1]))
...
def displayTable(resultsToDisplay):
    header=["Normal Form", "Total AFD", "0 < AFD < 0.5", "0.5 <= AFD < 0.75 ", "0.75 <= AFD <= 0.99 ", "AFD = 1"]
