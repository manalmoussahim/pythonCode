...
def executeFDDoscoveryQueryForThirdNF(queryElement, queryElementList, parameters):
    ...
    for otherNonKeyAttribute in substractLists(parameters[2],queryElementList):
        ...
        query="select (select count(*)  from (select "+queryElement+" from "+parameters[0]+" group by "+queryElement+" having count(distinct("+otherNonKeyAttribute+"))=1) as DerivedTable)::numeric/ (select count(distinct("+queryElement+")) from "+parameters[0]+") As VALUE"
        result=executeQuerry(query)[0][0]
        if result!=1 and result!=0:
            secondNFresults.append(result) 
        elif result==1: 
            fdsElements.append([queryElementList, otherNonKeyAttribute])

if __name__ == '__main__':
    connect()
    secondNFResults=meausre3NFLevel(["test",["k1","k2"], ["a","b","c"]])
    print(secondNFResults)